...
def validate(args, parser):
    if (args.output_h1 is None) and (args.output_h2 is None) and (args.output_h3 is None) and (args.output_h4 is None) and (args.output_untagged is None):
        parser.error(
            "Nothing to be done since neither --output-h1 nor --output-h2 nor --output-h3 nor --output-h4nor --output-untagged are given."
        )
...
def write_read_length_histogram(length_counts, histogram_file, exit_stack):
    ...
    all_read_lengths = sorted(itertools.chain(*(h1.keys(), h2.keys(), h3.keys(), h4.keys(), untag.keys())))
...
def run_split(
    ...
    with ExitStack() as stack:
        ...
        # TODO: obviously this won't work for more than four haplotypes
        ...
        input_reader, input_iterator, output_writers = initialize_io_files(
            reads_file, output_h1, output_h2, output_h3, output_h4, output_untagged, pigz, stack,
        )
